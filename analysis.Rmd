...
```{r}
m2fe <- lm_robust(f2, fixed_effects = cow, data = poly_varis)
...
```{r}
liplot2fe <- interflex(estimator = "linear", Y = "polyarchy_chg", X = "icap", D = "populismavg", Z = c("lnyears", "polyarchy_t0", "lngdppc", "lndemocexp", "lnseatshare", "op_strength", "lnopseatshare"), data = as.matrix(poly_varis), na.rm = T, wald = T, ylab = "Effect of Populist Discourse", xlab = "Incumbent Capacity", FE="cow")
...
```{r}
kiplot2fe <- interflex(estimator = "kernel", Y = "polyarchy_chg", X = "icap", D = "populismavg", Z = c("lnyears", "polyarchy_t0", "lngdppc", "lndemocexp", "lnseatshare", "op_strength", "lnopseatshare"), data = as.matrix(poly_varis), na.rm = T, wald = T, ylab = "Effect of Populist Discourse", xlab = "Incumbent Capacity", FE = "cow", main = "Figure 2: Effect of Populist Discourse on Electoral Democracy", theme.bw = T)
...
```{r}
poly_b <- lm(polyarchy_chg ~ populismavg + icap + lnseatshare + lndemocexp + lnyears + lngdppc + polyarchy_t0 + op_strength + lnopseatshare + factor(cow), data = poly_varis)

poly_i <- lm(polyarchy_chg ~ populismavg * icap + lnseatshare + lndemocexp + lnyears + lngdppc + polyarchy_t0 + op_strength + lnopseatshare + factor(cow), data = poly_varis)

poly_lr <- lrtest(poly_b, poly_i)
poly_lr
```
...
```{r}
ff_b <- lm(frefair_chg ~ populismavg + icap + lnseatshare + lndemocexp + lnyears + lngdppc + frefair_t0 + op_strength + lnopseatshare + factor(cow), data = ff_varis)

ff_i <- lm(frefair_chg ~ populismavg * icap + lnseatshare + lndemocexp + lnyears + lngdppc + frefair_t0 + op_strength + lnopseatshare + factor(cow), data = ff_varis)

ff_lr <- lrtest(ff_b, ff_i)
ff_lr
```
...
```{r}
kiplot6fe <- interflex(estimator = "kernel", Y = "civlib_chg", X = "icap", D = "populismavg", Z = c("lnyears", "civlib_t0", "lngdppc", "lndemocexp", "lnseatshare", "op_strength", "lnopseatshare"), data = as.matrix(cl_varis), na.rm = T, wald = T, Dlabel = "Logged Populism", Xlabel = "Incumbent Capacity", FE = "cow")
...
```{r}
civlib_b <- lm(civlib_chg ~ populismavg + icap + lnseatshare + lndemocexp + lnyears + lngdppc + civlib_t0 + op_strength + lnopseatshare + factor(cow), data = cl_varis)

civlib_i <- lm(civlib_chg ~ populismavg * icap + lnseatshare + lndemocexp + lnyears + lngdppc + civlib_t0 + op_strength + lnopseatshare + factor(cow), data = cl_varis)

civlib_lr <- lrtest(civlib_b, civlib_i)
civlib_lr
```
...
m8fe <- lm_robust(f8, fixed_effects = cow, data = ha_varis)

summary(m8fe)

liplot8fe <- interflex(estimator = "linear", Y = "horacc_chg", X = "icap", D = "populismavg", Z = c("lnyears", "horacc_t0", "lngdppc", "lndemocexp", "lnseatshare", "op_strength", "lnopseatshare"), data = as.matrix(ha_varis), na.rm = T, wald = T, Dlabel = "Logged Populism", Xlabel = "Incumbent Capacity", FE = "cow")

print(liplot8fe$figure)

kiplot8fe <- interflex(estimator = "kernel", Y = "horacc_chg", X = "icap", D = "populismavg", Z = c("lnyears", "horacc_t0", "lngdppc", "lndemocexp", "lnseatshare", "op_strength", "lnopseatshare"), data = as.matrix(ha_varis), na.rm = T, wald = T, Dlabel = "Logged Populism", Xlabel = "Incumbent Capacity", FE = "cow")
...
```{r}
ha_b <- lm(horacc_chg ~ populismavg + icap + lnseatshare + lndemocexp + lnyears + lngdppc + horacc_t0 + op_strength + lnopseatshare + factor(cow), data = ha_varis)

ha_i <- lm(horacc_chg ~ populismavg * icap + lnseatshare + lndemocexp + lnyears + lngdppc + horacc_t0 + op_strength + lnopseatshare + factor(cow), data = ha_varis)

ha_lr <- lrtest(ha_b, ha_i)
ha_lr
```
...
plot_base <- ggplot(base_mat, aes(x = r_names, y = mean, ymin = lower, ymax = upper, group = type, alpha = type, shape = type)) +
  ...
  labs(title = "Figure 1: Substantive Effects on Backsliding", x = "", y = "Predicted Effect", caption = "All predictors are mean-centered and standarized to a two standard deviation change") +
...
ggsave("/populistpaths/plots/baseline sub.png", width = 4, height = 3, plot = plot_base)
